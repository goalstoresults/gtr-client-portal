...
export async function renderContactDiagnostics(setupContent, portalState) {
  ...
  if (!project) {
    setupContent.innerHTML = `
      <section class="card">
        <p>Please select a project in the Client tab before using Contact Diagnostics.</p>
      </section>
    `;
    return;
  }
...
async function loadContacts(project, portalState) {
  ...
  try {
    const filterSelect = document.getElementById("cd-filter");
    const filter = filterSelect ? filterSelect.value || "missing_crm" : "missing_crm";
...
function renderRows(portalState) {
  ...
  let rows = sortRows([...CD_CACHE], portalState);
...
function exportSelected() {
  const selected = [...document.querySelectorAll(".cd-row:checked")].map(
    (cb) => {
