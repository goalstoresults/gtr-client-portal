// js/setup/tab-contact-list.js
// v3.0 — Contact List Setup Subtab (extracted from legacy setup.js)
...
// Shared save helper (same logic as original)
async function saveContactListSetup(portalState, gridBody) {
  const rows = [];
  ...
  alert("Contact List configuration saved.");
}

export async function renderContactListSetup(container, portalState) {
  ...
  if (configured.length > 0) {
    ...
    defaultBtn.addEventListener("click", () => {
      const defaults = [
        ...
        { field_key: "business_name", label: "Business", sort_order: 20, section: "General" },
        { field_key: "email", label: "Email", sort_order: 30, section: "General" },
        { field_key: "phone", label: "Phone", sort_order: 40, section: "General" },
        { field_key: "contact_type", label: "Type", sort_order: 50, section: "General" }
      ];
  ...
  container.querySelector("#btnSaveListConfig").addEventListener("click", async () => {
    await saveContactListSetup(portalState, gridBody);
  });
}
