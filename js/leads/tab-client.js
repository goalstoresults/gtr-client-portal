...
function renderClientForm(container, contact, portalState) {
  ...
  container.innerHTML = `
    <section class="card" style="margin-top:20px;">
      ...
      <button id="btnSaveClient" class="btn-primary" style="margin-top:16px;">
        Save
      </button>
    </section>
  `;
...
async function saveClient(existing, portalState) {
  ...
  try {
    ...
    await fetch(
      `https://leads-module.dennis-e64.workers.dev/leads/edit/${portalState.activeLeadId}`,
      {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
    ...
    alert("✅ Client saved and linked to lead.");

    // Update blue bar
    const bar = document.getElementById("lead-context-bar");
    bar.textContent = `${payload.first_name} ${payload.last_name}`;
